...
#-------------------------------------------------------------------
midi = pretty_midi.PrettyMIDI('/Users/jihoon/Desktop/Capstone 2026/SNNTraining/sonata-for-two-pianos-in-d-major-k448375a.mid')

groups = {}

duration = 30

# collect notes from first duration seconds
for instrument in midi.instruments:
    for note in instrument.notes:
        ...
        if note.start <= duration:
            ...
            groups[t].append(note.pitch)

# store time and highest_pitch)
result = []

for t in sorted(groups.keys()):
    highest_pitch = max(groups[t])
    result.append((t, highest_pitch))
...
for arr in result:
    time_of_note.append(float(arr[0]))
    note.append(arr[1])

#spike train conversion
note_diff = np.diff(note, axis=0)

music_spike_train = []
time_of_music_spike_train = []

for i in range(0,len(note_diff)):
    diff = note_diff[i]

    if diff > 0:
        music_spike_train.append(1)
        time_of_music_spike_train.append(time_of_note[i+1])
    elif diff < 0:
        music_spike_train.append(-1)
        time_of_music_spike_train.append(time_of_note[i+1])

#classify negative vs positive
music_positive_spike_train_o = []
music_negative_spike_train_o = []

for spike, time in zip(music_spike_train,time_of_music_spike_train):
    if spike == 1:
        music_positive_spike_train_o.append(time)
    else:
        music_negative_spike_train_o.append(time)

#add time
music_positive_spike_train = [x + 180 for x in music_positive_spike_train_o]
music_negative_spike_train = [x + 180 for x in music_negative_spike_train_o]

#eeg channel connection
input_group_ME = SpikeGeneratorGroup(1, indices=[0]*len(music_positive_spike_train),times=np.array(music_positive_spike_train) * second)
input_ME = Synapses(input_group_ME, E, on_pre='v_post += 1.5')
input_ME.connect(i=0,j=[0, 1, 8,9])

input_group_MI = SpikeGeneratorGroup(1, indices=[0]*len(music_negative_spike_train),times=np.array(music_negative_spike_train) * second)
input_MI = Synapses(input_group_MI, I, on_pre='v_post += 1.5')
input_MI.connect(i=0,j=[0,1,3,4])
#-------------------------------------------------------------------
...
'''
#plot simulation
mask = (spikemon_E.t > 130*second) & (spikemon_E.t < (180+duration)*second)
